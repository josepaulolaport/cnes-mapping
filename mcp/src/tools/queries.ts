...
export async function listTables(client: pg.PoolClient): Promise<object> {
  ...
  for (const row of result.rows) {
    try {
      const c = await client.query(
        `SELECT COUNT(*)::int AS n FROM "${SCHEMA}"."${row.table_name}"`
      );
...
export async function describeTable(client: pg.PoolClient, tableName: string): Promise<object> {
  ...
  const countResult = await client.query(
    `SELECT COUNT(*)::int AS n FROM "${SCHEMA}"."${safeName}"`
  );

  let sampleRows: unknown[] = [];
  try {
    const sample = await client.query(
      `SELECT * FROM "${SCHEMA}"."${safeName}" LIMIT 3`
    );
...
export async function getRelationships(client: pg.PoolClient, tableName?: string): Promise<object> {
...
export async function searchSchema(client: pg.PoolClient, keyword: string): Promise<object> {
...
export async function executeQuery(client: pg.PoolClient, sql: string): Promise<object> {
