...
function validateCode(code) {
    if (!code.trim())
        throw new Error("Analysis code cannot be empty");
...
export async function runAnalysis(input) {
    ...
    if (!dbUrl)
        throw new Error("CNES_DATABASE_URL not configured");
...
function execPython(scriptPath, outputDir, dbUrl) {
...
function collectArtifacts(outputDir) {
    ...
    if (paths.length === 0) {
        for (const name of readdirSync(outputDir)) {
            if (name === "analysis.py" || name === "artifacts.json")
                continue;
            paths.push(name);
        }
    }
...
export function listAnalysisRuns(limit = 20) {
    ...
    const runs = readdirSync(RUNS_DIR)
        ...
        const artifacts = readdirSync(dir).filter((f) => !f.endsWith(".py") && f !== "artifacts.json");
        return {
            run_id: runId,
...
export function getAnalysisGuide() {
    return {
        purpose: "For deep analysis over large CNES datasets, write Python code instead of pulling rows via execute_query.",
        when_to_use: [
