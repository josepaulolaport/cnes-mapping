...
class DermETL:
    ...
    # ------------------------------------------------------------------
    # Representatives (high-assertion only)
    # ------------------------------------------------------------------
    def _insert_representatives(self, rows: list[dict[str, Any]]) -> int:
        if self.dry_run or not rows:
            return 0
        rdf = pd.DataFrame(rows).drop_duplicates(["facility_id", "external_source_key"])
        with self.engine.begin() as conn:
    ...
    def fill_responsible_names(self, facility_map: Optional[pd.DataFrame] = None) -> int:
        ...
        est = est[est["CO_UNIDADE"].isin(unit_ids)].dropna(subset=["REG_DIRETORCLN"])
        est["reg"] = est["REG_DIRETORCLN"].map(
            lambda x: (digits(x) or "").lstrip("0") or None
        )
        est = est.dropna(subset=["reg"])
